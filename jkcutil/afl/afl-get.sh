...
#   ./jkcutil/afl/afl-get.sh <directory to build and install AFL in>
...
echo "You can now build junkcoin with AFL instrumentation as follows:"
echo "$ make clean # if you've already built bitcoinzd without AFL instrumentation"
echo "$ ./jkcutil/afl/afl-build.sh '$(pwd)' <fuzz case> -j\$(nproc)"

...
PACKAGE_VERSION=$($SRC_PATH/src/junkcoind --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/junkcoind $DEB_BIN
cp $SRC_PATH/src/junkcoin-cli $DEB_BIN
...
# Copy manpages
cp $SRC_DOC/man/junkcoind.1 $DEB_MAN
cp $SRC_DOC/man/junkcoin-cli.1 $DEB_MAN
cp $SRC_DOC/man/zcash-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/junkcoind.bash-completion $DEB_CMP/junkcoind
cp $SRC_PATH/contrib/junkcoin-cli.bash-completion $DEB_CMP/junkcoin-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/junkcoind.1
gzip --best -n $DEB_MAN/junkcoin-cli.1
gzip --best -n $DEB_MAN/zcash-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/junkcoind $DEB_BIN/junkcoin-cli

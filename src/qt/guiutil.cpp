...
{
    // return if URI is not valid or is no junkcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("jkc"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("junkcoin://", Qt::CaseInsensitive))
    {
...
{
    QString ret = QString("junkcoin:%1").arg(info.address);
    int paramCount = 0;

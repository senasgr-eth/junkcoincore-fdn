...
Below are some notes on how to build Junkcoin Core for Windows.
...
Note that for WSL v1 the Junkcoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/junkcoin, AND not under, for example, /mnt/d/junkcoin. 
...
way. This will install to `c:\workspace\junkcoin`, for example:

    make install DESTDIR=/mnt/c/workspace/junkcoin

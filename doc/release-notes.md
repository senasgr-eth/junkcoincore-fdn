JunkCoin Core version 1.14.6 is now available from:
...
changes to network efficiency. All Junkcoin Core users - miners, services,
...
Junkcoin network from 1 JKC to 0.01 JKC. The full recommendation can be found
...
1. The wallet will attempt to not create any outputs smaller than 0.03 JKC as
   change if possible.
2. If after signing there is more than 0.01 JKC left in the change output, the
   wallet will keep the output as-is.
3. If the change output drops under 0.01 JKC, the wallet will discard the
   change and instead give it to miners as fee.
4. Any output under 0.01 JKC will not be accepted as valid by default.
...
efforts to make Junkcoin more accessible to people that do not speak English.
...
* Improved and updated the [bip list](bips.md) documentation that explains the
  protocol features implemented by Junkcoin Core.
